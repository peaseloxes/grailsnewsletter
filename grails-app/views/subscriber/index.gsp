		...
		<div id="list-subscriber" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="unsubscribed" title="${message(code: 'subscriber.unsubscribed.label', default: 'Unsubscribed')}" />
					</tr>
				</thead>
				<tbody>
				<g:each in="${subscriberInstanceList}" status="i" var="subscriberInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: subscriberInstance, field: "name")}</td>
						<td>${fieldValue(bean: subscriberInstance, field: "unsubscribed")}</td>
					</tr>
				</g:each>
				</tbody>
			</table>
